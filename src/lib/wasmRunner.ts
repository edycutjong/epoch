...
// Lazily-compiled module for the Egress Dispatcher ("Blind Courier") contract.
...
// Synchronously invoke an exported function of the Egress Dispatcher contract,
// passing a JSON payload and returning its JSON result. This is what makes the
// host_contracts_call interface a *real* cross-contract TEE invocation: a second,
// independently-compiled WASM module runs inside the same host transaction and
// shares the tenant's host state (KV, stash, egress log).
function invokeExecutorContract(fnName: string, payload: string): string {
  const execModule = getExecutorModule();

  let execMemory: WebAssembly.Memory;
  const imports = { env: makeHostImports(() => execMemory) };
  const instance = new WebAssembly.Instance(execModule, imports);
  execMemory = instance.exports.memory as WebAssembly.Memory;
  ...
  const fn = instance.exports[fnName] as ((ptr: number, len: number) => bigint) | undefined;
  if (!fn) {
    throw new Error(`Egress Dispatcher does not export function "${fnName}".`);
  }

  const bytes = new TextEncoder().encode(payload);
  const ptr = allocFn(bytes.length);
  new Uint8Array(execMemory.buffer, ptr, bytes.length).set(bytes);
  ...
  const out = new TextDecoder().decode(new Uint8Array(execMemory.buffer, resPtr, resLen));
  deallocFn(resPtr, resLen);
  return out;
}

// Build the full T3 ADK host-import surface bound to a specific WASM instance's
// memory. Shared by the Switch Coordinator and the Egress Dispatcher so both
// contracts see the same simulated host state.
function makeHostImports(getMemory: () => WebAssembly.Memory) {
  const readStringFromWasm = (ptr: number, len: number): string => {
    const memView = new Uint8Array(getMemory().buffer, ptr, len);
  ...
  const writeStringToWasm = (str: string, ptr: number, maxLen: number): number => {
    ...
    const memView = new Uint8Array(getMemory().buffer, ptr, maxLen);
  ...
  return {
    ...
    host_stash_put: (dataPtr: number, dataLen: number, refBufPtr: number, refBufLen: number): number => {
      const memView = new Uint8Array(getMemory().buffer, dataPtr, dataLen);
    ...
    host_stash_get: (refPtr: number, refLen: number, dataBufPtr: number, dataBufLen: number): number => {
      ...
      const memView = new Uint8Array(getMemory().buffer, dataBufPtr, dataBufLen);
    ...
    host_contracts_call: (
      contractPtr: number, contractLen: number,
      fnPtr: number, fnLen: number,
      payloadPtr: number, payloadLen: number,
      resBufPtr: number, resBufLen: number
    ): number => {
      const contractId = readStringFromWasm(contractPtr, contractLen);
      const fnName = readStringFromWasm(fnPtr, fnLen);
      const payload = readStringFromWasm(payloadPtr, payloadLen);
      console.log(`[Host ContractsCall] Cross-contract invoke ${contractId}.${fnName}() inside TEE tx`);

      try {
        const resultJson = invokeExecutorContract(fnName, payload);
        console.log(`[Host ContractsCall] ${contractId}.${fnName}() returned: ${resultJson}`);
        return writeStringToWasm(resultJson, resBufPtr, resBufLen);
      } catch (e: any) {
        console.error(`[Host ContractsCall] Invocation of ${contractId}.${fnName}() failed: ${e.message}`);
        return -1;
      }
    },

    host_outbox_enqueue: (
      idkPtr: number, idkLen: number,
      payloadPtr: number, payloadLen: number
    ): number => {
      const idk = readStringFromWasm(idkPtr, idkLen);
      const payload = readStringFromWasm(payloadPtr, payloadLen);

      const db = readDb();
      db.outbox = db.outbox || [];
      // At-least-once with idempotency: dedupe on the idempotency key (idk).
      const exists = db.outbox.find((e: any) => e.idk === idk);
      if (!exists) {
        db.outbox.push({ idk, payload, enqueuedAt: Date.now(), status: 'enqueued' });
        writeDb(db);
        console.log(`[Host Outbox] Durably enqueued event idk=${idk}`);
      } else {
        console.log(`[Host Outbox] Duplicate idk=${idk} ignored (idempotent)`);
      }
      return 0;
    }
  };
...
export async function runWasmContract(
  ...
  // Host imports object (Switch Coordinator instance)
  const importObject = {
    env: makeHostImports(() => wasmMemory)
  };
  ...
  // Helper for reading the final result back out of the Coordinator memory
  const readStringFromWasm = (ptr: number, len: number): string => {
